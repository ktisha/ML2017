...
\begin{frame}{Оценивание информативности}
	...
	\begin{enumerate}
		...
		\item $I(tp, fp) = Accuracy = tp-fp \rightarrow \max$
...
\begin{frame}{Энтропия Шеннона}
  ...
  $p_i$ -- процентное соотношение $i$-го класса в разделении\\
  \pause
  \TODO{картинка с примером из датасета }
\end{frame}

\begin{frame}{Критерий информационного выигрыша}
  $$H = \sum\limits_{i=1}^{C} p_i \log_2 p_i$$\\
  \begin{align*} 
		IGain(X, x^j) &= H(X) - \sum \limits_{v \in vals(x^j)} \frac{\vert\{x \in X| x^j=v\}\vert}{\vert X \vert} H(\{x \in X| x^j=v\})\\
	\end{align*} 
\end{frame}
...
\begin{frame}\frametitle{Что почитать по этой лекции}
  \begin{itemize}
    ...
    \item \href{https://habrahabr.ru/post/321216/}{Хабр про Affinity Propagation}
  \end{itemize}
\end{frame}
